...
TVector& TVector::operator =(const TVector& value)
{
	if (this!=&value)
	{
		if(n!=value.n)
		{
			//åñëè ïàìÿòü óæå âûäåëåíà,óäàëèòü åå
			if (data) {delete[] data;}
			//âûäåëåíèå íîâîé ïàìÿòè
			data=new long double[value.n];
			//ñîõðàíåíèå íîâîãî ðàçìåðà
			n=value.n;
		}
		//ïåðåíîñ äàííûõ èç ïðàâîãî îïåðàíäà â ëåâûé
...
TMatrix TVector::inMatrix() const
{
	try
			{
                if (static_cast<int>((sqrt(this->GetSize())))!=static_cast<int>(sqrt(this->GetSize()))) throw 1;
			}
			catch (std::exception& ex)
			{
                  ex.what();
			}
            TMatrix A(static_cast<int>(sqrt(this->GetSize())),static_cast<int>(sqrt(this->GetSize())));
            for(int j =0;j<static_cast<int>(sqrt(this->GetSize()));j++)
					{
                        for(int i =0;i<static_cast<int>(sqrt(this->GetSize()));i++)
							{
...
{
    TVector vect(static_cast<int>((this->GetSize()+a.GetSize())));
			for(int i=0;i<this->GetSize();i++)
			{
...
{
	//yaw - ðûñêàíüå; pitch - òàíãàæ; roll - êðåí
...
// Êîíñòðóêòîð ñ çàäàííîé ðàçìåðíîñòüþ
...
{
	this->n=n;// ïîëó÷åíèå ñòðîê
	this->m=m; //ïîëó÷åíèå ñòîëáöîâ
...
// Åñëè ëåâûé îïåðàíä íå ñîâïàäàåò ñ ïðàâûì
if (this != &rvalue) {
// Óäàëåíèå ðàíåå âûäåëåííîé ïàìÿòè
this->~TMatrix();
// Âûäåëåíèå íîâîé ïàìÿòè ïî ðàçìåðàì ïðàâîãî îïåðàíäà
this->Resize(rvalue.n, rvalue.m);
// Ïåðåíîñ äàííûõ èç ïðàâîãî îïåðàíäà â ëåâûé ïîñòðî÷íî
for (int i = 0; i < n; i++)
memcpy(this->data[i], rvalue.data[i], sizeof(long double)*static_cast<unsigned>(m));
}
// Âîçâðàò ññûëêè íà ëåâûé îïåðàíä äëÿ âîçìîæíîñòè öåïî÷êè ïðèñâàèâàíèé
return (*this);
}
...
// Ïðîèçâîäÿùàÿ ôóíêöèÿ äëÿ ôîðìèðîâàíèÿ åäèíè÷íîé ìàòðèöû
...
{
	            try {
                if (e.GetSize()!=3)  throw "Íåâåðíî çàäàí âåêòîð îñè ïîâîðîòà";
				this->q0=cos(phi/2);
		    	Q=e;
		    	Q=Q.norm();
		    	Q=Q*(sin(phi/2));
...
TQuaternion::TQuaternion(long double yaw,long double pitch,long double roll)//çàäàåòñÿ â ãðàäóñíîé ìåðå
    //yaw - ðûñêàíüå; pitch - òàíãàæ; roll - êðåí
    {
...
TQuaternion TQuaternion::toReturnQ(const TMatrix& A) const
{
	try {
		...
		{
            ex.what();
            std::cout << "íå âîññòàíàâëèâàåò êâàòåðíèîíà ñ Ë0=0" <<std::endl;
			TQuaternion Q;
			return Q;
		}
}
